...
class OrderHolder with HavingSubscriptionMixin{
  ...
  OrderHolder() {
    ...
    finalPrice.bindTo(Observable.combineLatest2<double,double,double>(orderItems.producer.map((items) => items
        .map((order) => order.price.value * order.quantity.value)
        .toList()
        .reduce((price1, price2) => price1 + price2)), deliveryFee.producer, (maxPrice, fee) => maxPrice + fee));
